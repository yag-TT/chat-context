...
First check whether `.opencode-multi-agent/clonedeps.json` exists.

If it exists:

1. Read it before asking librarian for a new plan.
2. Check whether each listed `path` exists under `.opencode-multi-agent/clonedeps/repos/`.
...
```gitignore
# BEGIN opencode-multi-agent clonedeps
.opencode-multi-agent/clonedeps/repos/
# END opencode-multi-agent clonedeps
```
...
!.opencode-multi-agent/clonedeps.json
!.opencode-multi-agent/clonedeps/
!.opencode-multi-agent/clonedeps/repos/
!.opencode-multi-agent/clonedeps/repos/**
.opencode-multi-agent/clonedeps/repos/**/.git/
.opencode-multi-agent/clonedeps/repos/**/.git/**
# END opencode-multi-agent clonedeps
```
...
```text
.opencode-multi-agent/clonedeps/repos/<safe-repo-name>/
```
...
4. Clone into a temporary directory under `.opencode-multi-agent/clonedeps/repos/`, then move it
...
Write `.opencode-multi-agent/clonedeps.json` so future agents know what exists:
  ...
  "dependencies": [
    {
      ...
      "ref": "v1.3.17",
      "path": ".opencode-multi-agent/clonedeps/repos/opencode-ai__opencode",
...
Do not add `.opencode-multi-agent/clonedeps.json` to `.gitignore`. It is small, reviewable
project metadata that can be committed. Only the cloned repository contents
under `.opencode-multi-agent/clonedeps/repos/` should be ignored.
...
`.opencode-multi-agent/clonedeps/repos/` for inspection. Do not edit these clones.

- `.opencode-multi-agent/clonedeps/repos/<safe-name>/` - `<repo>` at `<ref>`; <one sentence on
  why this source is useful>.
- `.opencode-multi-agent/clonedeps/repos/<safe-name-2>/` - `<repo>` at `<ref>`; <one sentence on
  why this source is useful>.
```

Also keep `.opencode-multi-agent/clonedeps.json` updated as the structured manifest, but do not
...
Ask before removing `.opencode-multi-agent/clonedeps.json` or the `AGENTS.md` section because
