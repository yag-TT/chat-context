...
description: Configure and improve opencode-multi-agent for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# opencode-multi-agent Configuration Skill
...
- `~/.config/opencode/opencode-multi-agent.json` or `.jsonc`
...
opencode-multi-agent is configured through a plugin config file, usually:
...
| `~/.config/opencode/opencode-multi-agent.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/opencode-multi-agent.json` | User plugin config generated by installer |
| `<project>/.opencode/opencode-multi-agent.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/opencode-multi-agent/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/opencode-multi-agent/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/opencode-multi-agent/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/opencode-multi-agent/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
| `~/.config/opencode/skills/<skill-name>/SKILL.md` | Installed skill prompt payload |

This is a local-folder-only distribution. The generated config's `$schema`
must be the absolute `file://` URL for
`<expanded-folder>/opencode-multi-agent.schema.json`. If that folder moves,
rerun `bun run install:local` from the new location. Do not replace the absolute
plugin entry with a package name or registry reference.

Legacy plugin registrations, config files, prompt folders, state, logs, and
environment variables are independent. Do not read, migrate, delete, or alias
them while configuring this plugin.
...
1. If a `preset` is active, check
   `~/.config/opencode/opencode-multi-agent/{preset}/` first.
2. Fall back to `~/.config/opencode/opencode-multi-agent/`.
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `opencode-multi-agent.json[c]`; use markdown prompt override files instead.
  ...
  "presets": {
    "openai": {
      ...
      "librarian": {
        ...
        "variant": "low",
        "skills": [],
        "mcps": ["chrome_gemini_search", "gh_grep"]
      }
    }
  }
}
...
```text
~/.config/opencode/opencode-multi-agent/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/opencode-multi-agent/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/opencode-multi-agent/orchestrator.md
```
...
1. **Inspect current setup**
   - Read the existing opencode-multi-agent config file.
...
~/.config/opencode/opencode-multi-agent/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your opencode-multi-agent config?
```
