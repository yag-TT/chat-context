...
function activationPrompt(task: string): string {
  return [
    ...
    '- before planning, delegation, or creating state, inspect existing `.gitignore` and `.ignore`; add only missing entries without duplicates: `.gitignore` must contain `.opencode-multi-agent/deepwork/`, and `.ignore` must contain `!.opencode-multi-agent/deepwork/` and `!.opencode-multi-agent/deepwork/**`; this keeps state git-local yet OpenCode-readable;',
    '- create/update a `.opencode-multi-agent/deepwork/` progress file;',
